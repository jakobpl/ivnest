...
class WatchlistTableViewCell: UITableViewCell {
    ...
    func configure(with watchlistItem: WatchlistItem) {
        symbolLabel.text = watchlistItem.symbol
        nameLabel.text = watchlistItem.name
        priceLabel.text = formatCurrency(watchlistItem.currentPrice)
        
        changeLabel.text = formatCurrency(watchlistItem.priceChange)
        changeLabel.textColor = watchlistItem.priceChange >= 0 ? .systemGreen : .systemRed
        
        changePercentLabel.text = String(format: "%.2f%%", watchlistItem.priceChangePercent)
        changePercentLabel.textColor = watchlistItem.priceChangePercent >= 0 ? .systemGreen : .systemRed
    }
